...
pca_components = 5  # Seçilecek PCA bileşeni sayısı
...
print(f"Random Forest (PCA Sonrası) - Accuracy: {accuracy_rf_pca}, Precision: {precision_rf_pca}, Recall: {recall_rf_pca}, F1-Score: {f1_rf_pca}")
...
resized_img_datas_original = np.array([resize(img, (min_height, min_width), anti_aliasing=True) for img in all_img_datas_original])
all_labels_original = all_labels[:len(resized_img_datas_original)]

X_train_orig, X_val_orig, y_train_orig, y_val_orig = train_test_split(resized_img_datas_original, all_labels_original, test_size=0.3, random_state=42)
