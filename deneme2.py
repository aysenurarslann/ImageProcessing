...
# Giriş verilerinin şekillerini kontrol etmek ve yeniden şekillendirmek için:
...
# Beklenen şekil: (num_samples, 64, 64, 1)
...
def evaluate_model(model, X_val, y_val):
    ...
    precision = precision_score(np.argmax(y_val, axis=1), y_pred_val_classes, average='weighted')
...
model.fit(X_train_exp, y_train_one_hot, epochs=10, validation_data=(X_val_exp, y_val_one_hot))
...
X_new_train_exp = X_new_train.reshape(X_new_train.shape[0], roi_images.shape[1], roi_images.shape[2], 1)
X_new_val_exp = X_new_val.reshape(X_new_val.shape[0], roi_images.shape[1], roi_images.shape[2], 1)
