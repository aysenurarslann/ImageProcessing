...
# Random Forest eğitim ve doğrulama doğruluklarını grafikte gösterme
def plot_random_forest_accuracy(X_train, X_val, y_train, y_val, title_prefix="Random Forest"):
    ...
    # Doğrulama doğruluğunu hesaplama
    y_val_pred = clf.predict(X_val.reshape(X_val.shape[0], -1))
    val_accuracy = accuracy_score(y_val, y_val_pred)

    # Doğrulukları grafikte gösterme
    plt.figure(figsize=(10, 5))
    plt.bar(['Eğitim Doğruluğu', 'Doğrulama Doğruluğu'], [train_accuracy, val_accuracy])
    plt.title(f'{title_prefix} Doğrulukları')
    plt.ylabel('Doğruluk')
    plt.ylim(0, 1)
    plt.show()
...
# Random Forest eğitim ve doğrulama doğruluklarını görselleştirme (PCA öncesi)
plot_random_forest_accuracy(X_train_orig, X_val_orig, y_train_orig, y_val_orig, "Random Forest (PCA Öncesi)")

# Random Forest eğitim ve doğrulama doğruluklarını görselleştirme (PCA sonrası)
plot_random_forest_accuracy(X_train, X_val, y_train, y_val, "Random Forest (PCA Sonrası)")
