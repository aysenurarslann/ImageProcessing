...
for day_folder in day_folders:
    ...
    all_img_datas.append(roi_images)
    all_labels.append(labels)

# Tüm günlerin verilerini birleştirme
all_img_datas = np.concatenate(all_img_datas, axis=0)
all_labels = np.concatenate(all_labels, axis=0)

# Eğitim ve doğrulama setlerini ayırma
X_train, X_val, y_train, y_val = train_test_split(all_img_datas, all_labels, test_size=0.3, random_state=42)

# Giriş verilerini yeniden şekillendirmek için:
X_train_exp = X_train.reshape(X_train.shape[0], X_train.shape[1], X_train.shape[2], 1)
X_val_exp = X_val.reshape(X_val.shape[0], X_val.shape[1], X_val.shape[2], 1)

# Şekil kontrolü:
print("Reshaped X_train shape:", X_train_exp.shape)
print("Reshaped X_val shape:", X_val_exp.shape)

# Beklenen şekil: (num_samples, height, width, 1)
...
history = model.fit(X_train_exp, y_train_one_hot, epochs=10, validation_data=(X_val_exp, y_val_one_hot))

# Eğitim ve doğrulama kayıplarını yazdırma
...
selector = SelectFromModel(clf, prefit=True, threshold=-np.inf)  # threshold=-np.inf ile tüm özelliklerin seçilmesini sağlar
...
print("Selected X_new_train shape:", X_new_train.shape)
print("Selected X_new_val shape:", X_new_val.shape)

# Yeniden şekillendirme için doğru band sayısını hesaplayın
num_selected_bands = X_new_train.shape[1] // (roi_images.shape[1] * roi_images.shape[2])
if num_selected_bands == 0:
    print("No bands were selected. Please check your SelectFromModel parameters.")
else:
    X_new_train_exp = X_new_train.reshape(X_new_train.shape[0], roi_images.shape[1], roi_images.shape[2], num_selected_bands)
    X_new_val_exp = X_new_val.reshape(X_new_val.shape[0], roi_images.shape[1], roi_images.shape[2], num_selected_bands)

    # Yeni şekilleri kontrol edin
    ...
    history = model.fit(X_new_train_exp, y_train_one_hot, epochs=10, validation_data=(X_new_val_exp, y_val_one_hot))

    # Eğitim ve doğrulama kayıplarını yazdırma (bant seçimi sonrası)
    train_loss_new = history.history['loss']
    val_loss_new = history.history['val_loss']
    print(f"Train Loss (Bant Seçimi Sonrası): {train_loss_new}")
    print(f"Validation Loss (Bant Seçimi Sonrası): {val_loss_new}")
