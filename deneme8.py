...
# ROI Görüntülerini Görselleştirme
def display_roi_images(roi_images, num_images=5):
    fig, axes = plt.subplots(1, num_images, figsize=(20, 5))
    for i, ax in enumerate(axes):
        if i < len(roi_images):
            ax.imshow(roi_images[i][:, :, 0], cmap='gray')  # İlk bantı görselleştiriyoruz
            ax.set_title(f"ROI Image {i + 1}")
        ax.axis('off')
    plt.show()


display_roi_images(roi_images)


# Görselleştirme için model performans metriklerini çubuk grafik olarak gösterme
